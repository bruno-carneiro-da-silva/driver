...
import com.tkx.driver.TrackerActivity;
...
public class HolderActiveRide {
    ...
    private void setgoToRideButton(){
        if(mData.getBookingStatus().equals("1005")){
            context.startActivity(new Intent(context, FareActivity.class).putExtra(""+ IntentKeys.BOOKING_ID, ""+mData.getBooking_id()));
        }else{
            context.startActivity(new Intent(context, TrackerActivity.class).putExtra(""+ IntentKeys.BOOKING_ID, ""+mData.getBooking_id()));
        }
