...
import com.tkx.driver.models.ModelPersonalDocument;
    ...
    public void onFetchComplete(Object script, String APINAME) {
        ...
        if (modelPersonalDocument.getData().size() > 0) {
            ...
            for (int i = 0; i < modelPersonalDocument.getData().size(); i++) {
                placeholder.addView(new HolderpersonalDocumentItem(
                        modelPersonalDocument.getData().get(i).getExpire_date_status(),
                        modelPersonalDocument.getData().get(i).getDocument_number_required(),
                        modelPersonalDocument.getData().get(i),
                        modelPersonalDocument.getData().get(i).getExpire_date()));
            }
        ...
        if (modelPersonalDocument.getVehicle_document().size() > 0) {
            ...
            for (int i = 0; i < modelPersonalDocument.getVehicle_document().size(); i++) {
                ...
                placeholder.addView(new HolderVehicleDocumentItem(
                        modelPersonalDocument.getVehicle_document().get(i)));
...
//        },1000);
    ...
    class HolderpersonalDocumentItem {
        ...
        public HolderpersonalDocumentItem(int expiry_date_status, int document_expiry_required, ModelPersonalDocument.DataBean dataBean, String expiry_date) {
            this.mDataBean = dataBean;
            this.expiry_date = expiry_date;
            this.expiry_date_status = expiry_date_status;
            this.document_expiry_required = document_expiry_required;
        ...
        private void OnClick() {
            ...
            if (mDataBean.getTemp_doc_upload()) {
                PersonalDocumentActivity.this.startActivity(new Intent(PersonalDocumentActivity.this, PhotoUploaderActivity.class)
                        ...
                        .putExtra(IntentKeys.DOCUMENT_NUMBER_REQUIRED, ""+document_expiry_required)
    ...
    class HolderVehicleDocumentItem {
        ...
        private void setdata() {
            ...
            for (int i = 0; i < vehicleDocumentBean.getVehicle_documents().size(); i++) {
                placeHolderNew.addView(new HolderVehicleDocumentItemNew(
                        vehicleDocumentBean.getVehicle_documents().get(i).getExpire_date_status(),
                        vehicleDocumentBean.getVehicle_documents().get(i).getDocument_number_required(),
                        vehicleDocumentBean.getVehicle_documents().get(i), vehicleDocumentBean.getId()));
            }
    ...
    class HolderVehicleDocumentItemNew {
        ...
        public HolderVehicleDocumentItemNew(int expiry_status, int document_number_required, ModelPersonalDocument.VehicleDocumentBean.VehicleDocumentsBean vehicleDocumentBean, int vehicle_id) {
        ...
        private void OnClick() {
            ...
            if (vehicleDocumentBean.getTemp_doc_upload()) {
                PersonalDocumentActivity.this.startActivity(new Intent(PersonalDocumentActivity.this, PhotoUploaderActivity.class)
                        .putExtra("" + IntentKeys.EXPIRY_DATE, ""+expiry_status)
                        .putExtra(IntentKeys.DOCUMENT_NUMBER_REQUIRED, "" + document_number_required)
                        .putExtra("" + IntentKeys.DRIVER_ID, "" + sessionManager.getUserDetails().get(SessionManager.KEY_DRIVER_ID))
