...
import com.tkx.driver.models.restmodels.NewRideAcceptmodel;
import com.tkx.driver.models.restmodels.NewRideRejectModel;
...
public class ReceiveRentalPassengerActivity extends BaseActivity implements ApiManager.APIFETCHER {
    ...
    @BindView(R.id.car_type_name_txt)
    TextView carTypeNameTxt;
    
    // @BindView(R.id.car_type_antt_txt)
    // TextView carTypeAnttTxt;
    
    @BindView(R.id.package_txt)
    TextView packageTxt;
    @BindView(R.id._time_of_booking_txt)
    TextView TimeOfBookingTxt;
    @BindView(R.id.eta_price_txt)
            ...
            switch (APINAME) {
                case Config.ApiKeys.KEY_REST_RIDE_INFO:
                    ...
                    if (rs.getStatus() == 1) {
                        ...
                        // carTypeAnttTxt.setText("" + response.getDetails().getCar_type_name());
                        ...
                        Glide.with(this).load("" + Apis.googleImage + "" + response.getDetails().getPickup_lat() + "," + response.getDetails().getPickup_long() + "&zoom=15&size=400x400&key=" + Config.getEncodedApiKey(getApplicationContext())).into(mapImage);
                        ...
                        Toast.makeText(this, "Algo deu errado com a API_S ", Toast.LENGTH_SHORT).show();
                    }
                    break;
