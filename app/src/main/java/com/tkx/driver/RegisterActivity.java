...
import static com.tkx.driver.ManualUserDetailActivity.isValidPhoneNumber;
...
public class RegisterActivity extends BaseActivity implements ApiManager.APIFETCHER {
    ...
    TextView phone_code;
    @BindView(R.id.edtCpfNumber)
    EditText edtCpfNumber;
    @BindView(R.id.llCpfNumber)
    LinearLayout llCpfNumber;
    @BindView(R.id.viewGender)
    View viewGender;
    @BindView(R.id.llEmail)
    LinearLayout llEmail;
    // @BindView(R.id.inputReferralCode)
    ...
    @BindView(R.id.subscription_plan)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        OneSignal.startInit(RegisterActivity.this)
        ...
        country_code = (CountryCodePicker)findViewById(R.id.country_code1);
        ...
        try {
            address.setVisibility(sessionManager.getAppConfig().getData().isAddress_details()?View.VISIBLE:View.GONE);
        ...
        driver_image.setOnClickListener(view -> {
            ...
            builderSingle.setTitle(R.string.upload_profile_pic);
            ...
            // .setFolderTitle(this.getResources().getString(R.string.albums)) // Folder title (works with FolderMode = true)
            // .setImageTitle(this.getResources().getString(R.string.albums)) // Image title (works with FolderMode = false)
            // .setDoneTitle(this.getResources().getString(R.string.done)) // Done button title
        ...
        area.setOnClickListener(view -> {
            if (Selected_Country_position == 9999999) {
                ...
                if (sessionManager.getAppConfig().getData().getCountries().get(Selected_Country_position).getCountry_area().size() <= 0) {
                    ...
                    builderSingle.setTitle(R.string.select_area);
        ...
        edt_email_signup.setOnClickListener(v -> {
            if (sessionManager.getAppConfig().getData().getRegister().isDriver_email_otp()) {
                startActivityForResult(new Intent(RegisterActivity.this, OTPVerifierActivity.class)
        ...
        if (sessionManager.getAppConfig().getData().getRegister().isEmail()) {
            edt_email_signup.setHint(getResources().getString(R.string.enter_email));
        } else {
            edt_email_signup.setHint(getResources().getString(R.string.enter_email_optional));
        }



        //  ll_register.setVisibility(sessionManager.getAppConfig().getData().getRegister().isEmail() ? View.VISIBLE : View.GONE);
        ...
        ll_register.setOnClickListener(v -> {
            ...
            if (BuildConfig.Build_varient.equals("twendedriver")){

            }else {
                if (!edt_email_signup.getText().toString().trim().matches(emailPattern) && sessionManager.getAppConfig().getData().getRegister().isEmail()) {
            ...
            // } 
            ...
            else if (BuildConfig.Build_varient.equals("drivadriver") && edt_phone.getText().toString().startsWith("0")) {
                Toast.makeText(RegisterActivity.this, "Insira o número de telefone sem zero", Toast.LENGTH_SHORT).show();
                ...
                data.put("number", "" + houseNo.getText().toString());
                data.put("street", "" + street_number.getText().toString());
                data.put("city", "" + city_name.getText().toString());
                data.put("state_code", "" + statecode.getText().toString());
                data.put("postCode_cep", "" + postalcode.getText().toString());
                try {
                ...
                if (sessionManager.getAppConfig().getData().getRegister().isDriver_phone_otp()) {
                    ...
                    data.put("phone", "" + sessionManager.getAppConfig().getData().getCountries().get(Selected_Country_position).getPhonecode() + edt_phone.getText().toString());
                    // data.put("phone", "" + country_code.getSelectedCountryCodeWithPlus() + edt_phone.getText().toString());
                }
                if (sessionManager.getAppConfig().getData().getRegister().isSmoker()) {
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        try {
            // carregar a câmera ao clicar no botão cadastrar
            // cameraTask();
        } catch (Exception e) {
        }
    }
    ...
    public void onFetchComplete(Object script, String APINAME) {
        try {
            if (APINAME.equals(API_S.Tags.REGISTER_DRIVER)) {
                modelRegister = SingletonGson.getInstance().fromJson("" + script, ModelRegister.class);
                ...
                sessionManager.createLoginSession(false,
                        ...
                        , ""
                        , modelRegister.getData().getPhoneNumber()
                ,modelRegister.getData().getNumber()
                ,modelRegister.getData().getStreet()
                ,modelRegister.getData().getCity()
                ,modelRegister.getData().getState_code()
                ,modelRegister.getData().getPostCode_cep());
                ...
                //                if(modelRegister.getData().getPush_notification().isFire_base()==true){
                //
                //
                //                    HashMap<String, String> data = new HashMap<>();
                //
                ////                    if(modelRegister.getData().getPush_notification().isFire_base()==true){
                ////                        sessionManager.setFirebaseNotification(modelRegister.getData().getPush_notification().isFire_base());
                ////                        data.put("player_id", "" + FirebaseInstanceId.getInstance().getToken());
                ////                    }else{
                ////                        data.put("player_id", "" + status.getSubscriptionStatus().getUserId());
                ////                    }
                //
                ...
                //
                //                    try {
                //                        apimanager._post_with_secreteonly(API_S.Tags.UPDATE_PLAYER_ID, API_S.Endpoints.UPDATE_PLAYER_ID, data);
                //                    } catch (Exception e) {
                //                        e.printStackTrace();
                //                    }
                //
            ...
            if (APINAME.equals(API_S.Tags.UPDATE_PLAYER_ID)) {
