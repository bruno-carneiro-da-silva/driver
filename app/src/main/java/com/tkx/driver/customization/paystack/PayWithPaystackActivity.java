package com.tkx.driver.customization.paystack;
...
import com.tkx.driver.models.ModelGetStripeToken;
...
//add text changed listener
        ...
        mEditExpiryMonth.addTextChangedListener(new PayWithPaystackActivity.ExpiryWatcher(mEditExpiryMonth));
        mEditExpiryYear.addTextChangedListener(new PayWithPaystackActivity.ExpiryWatcher(mEditExpiryYear));
...
//for now send the dummy response to the previous activity to get the payment done
    ...
    private void createToken(final Card card) {
        // PaystackSdk.setPublicKey("pk_live_608be012f830f3fe2d928cb10d1dab7086bc8bf8");
        PaystackSdk.setPublicKey(""+ BuildConfig.paystack);
        Charge charge = new Charge();

//        String currency =  MainActivity.mModelViewServcesAndCars.getCurrency();

        charge.setAmount(50);
        //charge.setCurrency(currency);
        charge.setCurrency("GHS");
        if(null== sessionmanager.getUserDetails().get(SessionManager.KEY_DriverEmail)){

            charge.setEmail("");
        }else{
            charge.setEmail(sessionmanager.getUserDetails().get(SessionManager.KEY_DriverEmail));
        }
...
// for verification.
                ...
                if(null == sessionmanager.getUserDetails().get(SessionManager.KEY_DriverEmail)){
                    ...
                    data.put("payment_email",sessionmanager.getUserDetails().get(SessionManager.KEY_DriverEmail));
                }
                try {
                    progressDialog.show();
                    apiManagerNew._post(API_S.Tags.SEND_STRIPE_TOKEN, API_S.Endpoints.SEND_STRIPE_TOKEN, data, sessionmanager);
...
// error occurs with OTP, you should still verify on server.
            ...
            public void onError(Throwable error, Transaction transaction) {
                progressDialog.dismiss();
                Toast.makeText(PayWithPaystackActivity.this, ""+"err"+transaction.getReference()+"trow"+error, Toast.LENGTH_SHORT).show();
